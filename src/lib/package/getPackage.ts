...
const getPackageData = async (packageName: string): Promise<NpmFormattedMetadata | undefined> => {
  try {
    ...
    console.log("Retrieving package data error", err);
    return undefined;
  }
};

export const getPackage = async (packageName: string) =>
  await getOrSet<NpmFormattedMetadata | undefined>(
